...
ESM2 입력은 `--max_len`으로 잘립니다. `--position_col`을 제공하면 모델은 해당 residue token의 embedding을 사용하고, 그 token이 truncation으로 잘리면 명시적으로 에러를 냅니다. 위치 컬럼을 쓰지 않는 경우에는 CLS embedding으로 fallback하므로 변이 하나의 신호가 긴 sequence representation에 희석될 수 있습니다.
